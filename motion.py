...
def mail():
    ...
    # sending the mail
    s.sendmail("sootkardungi@gmail.com", "ahanaak13@.com", message)
...
def isHumanHome():
    ...
    # defining microphone as source for input.
    with sr.Microphone() as source2:
        SpeakText("what's the password?")
        # wait for a second to let the recognizer
        # adjust the energy threshold based on
        # the surrounding noise level
        r.adjust_for_ambient_noise(source2, duration=0.2) #method

        # listens for the user's input
        #Using listen() to Capture Microphone Input
        audio2 = r.listen(source2)

        # Using g00gle to recognize audio
